              ...
              haml_tag('div.row') do
                haml_tag('div.col-sm-5') do
                  haml_tag(:p) do
                    ...
                    haml_concat("IMDB Rating #{'%.1f' % movie['imdbRating']}")
                  end
                end
                haml_tag('div.col-sm-5') do
                  haml_tag(:p) do
                    haml_tag(:a, { href: '/rating/' + movie['id'].to_s }) do
                      haml_tag('i.fas.fa-ticket-alt', { class: ('kaleb-yellow' if movie['imdbRating'] >= 7.5) })
                      haml_concat("KMR User Rating #{'%.1f' % movie['imdbRating']}")
                    end
                  end
                end
              end
